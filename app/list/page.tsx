...
export default function List() {
  const [listByAlphabet, setListByAlphabet] = useState<Map<string, ListItem[]>>(new Map());
  useEffect(() => {
    fetch("/api/list").then(async (res) => {
      ...
      filteredList.map(itm => {
        alphabetMap.set(itm.name[0], [...(alphabetMap.get(itm.name[0]) || []), itm]);
      })
  ...
  console.log('Array.from(listByAlphabet.entries()) = ', Array.from(listByAlphabet.entries()))
